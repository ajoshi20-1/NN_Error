...
The notebooks assume the package source is available from the repository `src/` directory and use notebook-local helper scripts for data loading and preprocessing.
...
Example data files are stored under `notebooks/data/`. The BEPS examples use `notebooks/data/BEPS_data/PTO_BEPS_0p85um.npz`; the STM examples use the SXM and 3DS files in `notebooks/data/`.
